...
export default function ForgotPasswordPage() {
  ...
  const handleSend = useCallback(async (e?: React.FormEvent) => {
    e?.preventDefault();
    const { valid, errors } = validateForgotPasswordForm(email);
    if (!valid) { setEmailError(errors.email ?? ''); return; }
    setEmailError('');
    setLoading(true);
            ...
            <div className="p-6">
              <AnimatePresence mode="wait">
                {!sent ? (
                  <motion.div key="form" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}>
                    ...
                    <form onSubmit={handleSend} className="flex flex-col gap-4">
                      <div>
                        ...
                        />
                        {emailError && <p className="text-2xs mt-1" style={{ color: '#EF4444' }}>{emailError}</p>}
                      </div>
                  ...
                  <motion.div key="sent" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                    ...
                    {countdown > 0 ? (
                      <div className="w-full py-3 rounded-xl text-center"
                        ...
                        <p className="font-body text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>
                          Resend available in{' '}
                          <span className="font-mono" style={{ color: '#C9A84C' }}>{countdown}s</span>
                        </p>
                      </div>
                    ) : (
                    ...
                    <button onClick={() => { setSent(false); setEmail(''); setCountdown(0); }}
